...
STPrint()
{
  ...
  fprintf(table,
	  "********************************Symbol Table************************************\n\n");
  fprintf(table,
	  "          Name Nest-Level  Tree-Node Predefined        Kind       Type      Value  Offset Dimension size dim1 dim2\n\n");

  for (i = 1; i <= st_top; i++)
  {
    treeval = 0;
    fprintf(table, "%3d", i);
    for (attr_num = NAME_ATTR; attr_num <= DIM2_ATTR; attr_num++)
	...
	case VALUE_ATTR:
	  ...
	  /*case IsFormalAttr:
	  if (attr_val == 0)
	    fprintf(table, "%11s", "value arg");
	  else if (attr_val == 1)
	    fprintf(table, "%11s", "reference arg");
	    break;*/
	case DIMEN_ATTR:
	  fprintf(table, "%11d",attr_val);
	  break;
	case SIZE_ATTR:
	  fprintf(table, "%5d", attr_val);
	  break;
	case DIM1_ATTR:
	  fprintf(table, "%5d", attr_val);
	  break;
